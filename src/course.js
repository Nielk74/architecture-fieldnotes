...
window.addEventListener('fieldnotes:pip',event=>{const xp=totalXP();$('.map-pip').innerHTML=renderCourseBot({course:'fundamentals',xp,decorative:false});const stage=courseEvolutionFor('fundamentals',xp);$('#pip-name').textContent=stage.name;$('#pip-description').textContent='Earn XP in Fundamentals to unlock this course’s ten forms. Overall Pip grows across all books.';$('#pip-next').textContent=nextCourseEvolution('fundamentals',xp)?`Next course evolution at ${nextCourseEvolution('fundamentals',xp).xp} XP`:'All course forms unlocked!';$('.quest-emblem').title=stage.name+' · '+stage.description});
