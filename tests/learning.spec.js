...
test('Chapter one contributes earned XP to book progression without repeat rewards',async({page})=>{
 ...
 await expect(page.locator('#course-count')).toHaveText('5 of 8 complete');await page.getByRole('dialog').getByRole('button',{name:'Let’s go!'}).click();await page.locator('#show-map').click();await expect(page.locator('#pip-next')).toHaveText('Next course evolution at 120 XP');
});
